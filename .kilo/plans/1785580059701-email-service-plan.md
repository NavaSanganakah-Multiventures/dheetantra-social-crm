...
- Add `'email'` to `activeTab` union (line 14), NavItem "Email Service" (Mail icon) near Broadcast (line ~442), header title mapping (line 476), render `{activeTab === 'email' && <EmailServiceView />}` (line ~530), import the component.
- View sections (client component, same Tailwind conventions as `WhatsAppManagerView`, `useToast`):
  1. **Domains list**: status badge (pending/active/failed), mode chip, nameservers (full) or verification CNAME/TXT (cname) with copy buttons, DNS table (MX/SPF/DKIM/DMARC) after onboarding, actions: "Check again" (verify), "Delete" (delete with confirm).
