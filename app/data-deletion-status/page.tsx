...
function StatusContent() {
  ...
  return (
    <div className="w-full max-w-md bg-white dark:bg-surface-900 rounded-2xl shadow-sm border border-surface-200 dark:border-surface-800 p-8">
      <Link href="/" className="inline-flex items-center gap-2 text-surface-500 hover:text-surface-900 dark:hover:text-white transition-colors mb-8 text-sm font-medium">
        <ArrowLeft className="w-4 h-4" /> Back to Home
      </Link>
      
      <h1 className="text-2xl font-bold font-display text-surface-900 dark:text-white mb-2">Data Deletion Status</h1>
      ...
      <form className="mb-8" onSubmit={handleSubmit}>
        <div className="relative">
          ...
          <input 
            ...
            className="w-full pl-10 pr-4 py-3 bg-surface-50 dark:bg-surface-950 border border-surface-200 dark:border-surface-800 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all text-sm"
            required
          />
        </div>
        <button type="submit" className="w-full mt-4 bg-primary-600 hover:bg-primary-700 text-white font-medium py-3 rounded-xl transition-colors">
          Check Status
        </button>
      </form>

      {loading && (
        <div className="pt-6 border-t border-surface-200 dark:border-surface-800 text-center text-sm text-surface-500">
          Loading status...
        </div>
      )}
      ...
      {id && !loading && (
        <div className="pt-6 border-t border-surface-200 dark:border-surface-800">
          <h2 className="text-sm font-semibold text-surface-900 dark:text-white uppercase tracking-wider mb-4">Request Status</h2>
          ...
          {error ? (
            ...
            <div className="space-y-4">
              ...
              <div className="bg-surface-50 dark:bg-surface-950 p-4 rounded-xl text-xs text-surface-500 dark:text-surface-400 space-y-2">
...
export default function DataDeletionStatusPage() {
  return (
    <div className="min-h-screen bg-surface-50 dark:bg-surface-950 font-sans selection:bg-primary-500/30 flex flex-col items-center justify-center p-6">
      <Suspense fallback={<div className="text-sm text-surface-500">Loading...</div>}>
        <StatusContent />
      </Suspense>
    </div>
  );
