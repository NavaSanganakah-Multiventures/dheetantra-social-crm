...
// cannot be created") are NOT "already configured" Ã¢ÂÂ tolerating them would
...
// drops Ã¢ÂÂ so those are NOT adoptable.
function isUsableCatchAll(rule: any, workerName: string): boolean {
  ...
  return (rule.actions || []).some((a: any) => {
    ...
    // return a plain string Ã¢ÂÂ accept both so an existing rule is adopted
...
// configured worker (string or array action value Ã¢ÂÂ both API shapes are
...
export async function onboardDomain(env: any, row: any) {
  ...
  try {
    ...
    // 1. Create (or adopt an existing) Cloudflare zone if missing Ã¢ÂÂ idempotent
    if (!zoneId) {
      ...
      if (!created) {
        ...
        // Ã¢ÂÂ treat that as a race and adopt the winning zone instead of
    ...
    if (zone.status === 'active') {
      ...
      try {
        // Idempotent: onboarding runs repeatedly (admin approve + "Ã Â¤ÂÃ Â¤Â¾Ã Â¤ÂÃ Â¤ÂÃ Â¥ÂÃ Â¤Â" +
        // maintenance cron). Cloudflare returns "already enabled/exists"
        // errors when the state is already correct Ã¢ÂÂ those are NOT failures.
      ...
      if (routingReady) {
        ...
        // "add at your provider Ã¢ÂÂ not yet active".
...
// as gone Ã¢ÂÂ they are real failures.
...
export async function removeDomain(env: any, row: any) {
  ...
  if (!creds?.token) {
    if (row.zone_id) {
      // Cannot verify or clean up the live zone Ã¢ÂÂ deleting the row now would
      // orphan a zone that keeps routing inbound mail (silent drop) and let
      // another workspace re-adopt it later.
      zoneFailures.push({ status: 403, message: 'Cloudflare credentials missing Ã¢ÂÂ the zone stays in Cloudflare' });
    } else {
    ...
    // kept for retry Ã¢ÂÂ the domain must stay fully functional. A rule orphaned
  ...
  // removes the zone in Cloudflare and retries the delete Ã¢ÂÂ the zone is then
    ...
    for (const row of results || []) {
      try {
        ...
        if (updated?.status === 'active') {
          ...
          // still initializing / awaiting nameserver activation Ã¢ÂÂ a normal
...
export async function handleIncomingEmail(message: any, env: any, ctx: any) {
  ...
  try {
    ...
    // Real-time broadcast Ã¢ÂÂ same `new_message` shape as WhatsApp so Flutter
