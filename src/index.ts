...
app.post('/api/whatsapp/webhook', async (c) => {
  try {
    ...
    // event) with a loud error log ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ a hard 503 here silently kills every
    ...
    if (appSecret) {
      ...
      if (!constantTimeEqual(signature, expected)) {
        console.warn('[WhatsApp] Webhook signature mismatch ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ignoring event', {
      ...
      console.error('[WhatsApp] WHATSAPP_APP_SECRET missing in SECRETS_KV ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ accepting webhook WITHOUT signature verification. Add WHATSAPP_APP_SECRET (Meta App Secret) to SECRETS_KV to enable verification.');
    }
    ...
    if (body.object === 'whatsapp_business_account') {
      for (const entry of body.entry) {
        for (const change of entry.changes) {
          ...
          // Field: 'calls' ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ Meta sends call events here
          // ==========================================
          if (change.field === 'calls') {
            ...
            console.log(`[Calling] ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ calls field handler FIRED. phone_number_id from payload: ${change.value.metadata?.phone_number_id}`);
            ...
            for (const callData of callsArray) {
              ...
              if (!config) {
                console.error(`[Calling] ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ No config found for phone_number_id: ${phoneNumberId}`);
                continue;
              }
              ...
              if (config.calling_enabled === 0) {
                console.log(`[Calling] ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ Calling is disabled for ${phoneNumberId}. Skipping incoming call.`);
                continue;
              }
                ...
                try {
                  ...
                  if (schedule.enabled) {
                    ...
                    if (!days.includes(dayOfWeek) || currentTime < startMin || currentTime > endMin) {
                      console.log(`[Calling] ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ Outside call schedule for ${phoneNumberId}. Day=${dayOfWeek}, Time=${currentHour}:${currentMin}, Schedule=${schedule.start_time}-${schedule.end_time}`);
              ...
              if (event === 'connect' || event === 'offer') {
                // Incoming call ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ save to DB + broadcast to frontend
                let contactId = '';
              ...
              `).bind(callId, config.workspace_id, contactId, phoneNumberId, callerNumber, 'voice',
                ...
                // reject bhej dete hain ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ caller ko busy tone milega, app par
                // ring/push nahi aayegi. Ye "oldest wins" hai: do calls ek
                // saath aayein (race) toh jo pehle insert hui wo ring karegi,
                // baaki sab busy ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ dono webhooks isi same answer par converge
                // karte hain, isliye ye deterministic hai.
                //
                // NOTE (default dialer): PSTN calls mein bhi yahi line-busy
                // concept use hoga ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ app default dialer banne par incoming
                ...
                if (direction !== 'BUSINESS_INITIATED') {
                  ...
                  if (activeCall) {
                    console.log(`[Calling] ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ Line busy (existing ${activeCall.status}: ${activeCall.id}) ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ auto-rejecting call ${callId} from ${callerNumber}`);
                    // Meta ko reject ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ caller ko WhatsApp jaisa busy tone milega
                    try {
                    ...
                    // Call log mein 'busy' status ke saath record ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ ring/push
                    ...
                    console.log(`[Calling] Call ${callId} marked busy ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ no ring, no push`);
                    continue;
                  }
                }
                ...
                try {
                  ...
                  console.log(`[Calling] ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ Broadcast response from DO: ${broadcastBody}`);
                } catch (e) {
                  console.error('[Calling] ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ Failed to broadcast incoming call:', e);
                }
                    ...
                    (async () => {
                      try {
                        ...
                        if (!tokens.results || tokens.results.length === 0) {
                          console.warn(`[Calling] No FCM tokens for workspace ${config.workspace_id} ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ incoming call push skipped`);
                          return;
                        }
                        ...
                        // Caller ka rich context banao ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ email aur last message bhi push me bhejo
                ...
                // Busy-rejected call ka terminate event baad mein aata hai ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ
                            ...
                            for (let start = 0; start < targets.length; start += CHUNK) {
                              ...
                              const sends = await Promise.allSettled(
                                chunk.map((row) =>
                                  ...
                                  c.env,
                                  row.token,
                                  `ÃÂÃÂÃÂÃÂ ÃÂÃÂÃÂÃÂ¤ÃÂÃÂÃÂÃÂ®ÃÂÃÂÃÂÃÂ ÃÂÃÂÃÂÃÂ¤ÃÂÃÂÃÂÃÂ¿ÃÂÃÂÃÂÃÂ ÃÂÃÂÃÂÃÂ¤ÃÂÃÂÃÂÃÂ¸ÃÂÃÂÃÂÃÂ ÃÂÃÂÃÂÃÂ¥ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ ÃÂÃÂÃÂÃÂ¤ÃÂÃÂÃÂÃÂ¡ ÃÂÃÂÃÂÃÂ ÃÂÃÂÃÂÃÂ¤ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ ÃÂÃÂÃÂÃÂ¥ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ ÃÂÃÂÃÂÃÂ¤ÃÂÃÂÃÂÃÂ² +${callerNumber}`,
                                  'ÃÂÃÂÃÂÃÂ ÃÂÃÂÃÂÃÂ¤ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ ÃÂÃÂÃÂÃÂ¤ÃÂÃÂÃÂÃÂªÃÂÃÂÃÂÃÂ ÃÂÃÂÃÂÃÂ¤ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ ÃÂÃÂÃÂÃÂ¥ÃÂÃÂÃÂÃÂ ÃÂÃÂÃÂÃÂ ÃÂÃÂÃÂÃÂ¤ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ ÃÂÃÂÃÂÃÂ¤ÃÂÃÂÃÂÃÂ WhatsApp ÃÂÃÂÃÂÃÂ ÃÂÃÂÃÂÃÂ¤ÃÂÃÂÃÂÃÂµÃÂÃÂÃÂÃÂ ÃÂÃÂÃÂÃÂ¥ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ ÃÂÃÂÃÂÃÂ¤ÃÂÃÂÃÂÃÂ¯ÃÂÃÂÃÂÃÂ ÃÂÃÂÃÂÃÂ¤ÃÂÃÂÃÂÃÂ¸ ÃÂÃÂÃÂÃÂ ÃÂÃÂÃÂÃÂ¤ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ ÃÂÃÂÃÂÃÂ¥ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ ÃÂÃÂÃÂÃÂ¤ÃÂÃÂÃÂÃÂ² ÃÂÃÂÃÂÃÂ ÃÂÃÂÃÂÃÂ¤ÃÂÃÂÃÂÃÂ®ÃÂÃÂÃÂÃÂ ÃÂÃÂÃÂÃÂ¤ÃÂÃÂÃÂÃÂ¿ÃÂÃÂÃÂÃÂ ÃÂÃÂÃÂÃÂ¤ÃÂÃÂÃÂÃÂ¸ ÃÂÃÂÃÂÃÂ ÃÂÃÂÃÂÃÂ¤ÃÂÃÂÃÂÃÂ¹ÃÂÃÂÃÂÃÂ ÃÂÃÂÃÂÃÂ¥ÃÂÃÂÃÂÃÂ ÃÂÃÂÃÂÃÂ ÃÂÃÂÃÂÃÂ¤ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ ÃÂÃÂÃÂÃÂ¤ÃÂÃÂÃÂÃÂ',
                    ...
                    if (config && config.workspace_id) {
                      ...
                      if (members.results && members.results.length > 0) {
                        ...
                        if (tokens.results && tokens.results.length > 0) {
                          ...
                          console.warn(`[Webhook] No FCM tokens for workspace ${config.workspace_id} ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ push skipped`);
                        }
...
// ==========================================
// ---------------------------------------------------------------------------
// WebSocket auth helper ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ validates the auth_session cookie or a ?sid=...
...
async function getAuthenticatedUserForWs(c: Context<{ Bindings: Env }>): Promise<{ user: any; workspaceId: string } | null> {
  ...
  if (!c.env.DB) {
    // Local development fallback ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ still require a known room shape but skip DB.
...
const worker = {
  ...
  // via Meta API and updates campaign counters ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ see workers/broadcast-queue.ts)
