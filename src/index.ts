  ...
  async fetch(request: Request) {
    ...
    if (request.method === "POST" && url.pathname.endsWith("/broadcast")) {
      try {
        ...
        console.log(
          `[DO Broadcast] Sending type=${data.type} to ${sockets.length} WebSocket(s)`,
        );
        let sent = 0;
        ...
        console.log(
          `[DO Broadcast] Successfully sent to ${sent}/${sockets.length} WebSocket(s)`,
        );
    ...
    if (upgradeHeader === "websocket") {
      ...
      console.log(
        `[DO] WebSocket connected. Total sockets after accept: ${this.ctx.getWebSockets().length}`,
      );
  ...
  async webSocketMessage(ws: WebSocket, message: string | ArrayBuffer) {
    ...
    try {
      ...
      if (event === "offer" && payload.target) {
        console.log(
          `[Calling] Initiating call to WhatsApp target: ${payload.target}`,
        );
...
app.use("/api/*", async (c, next) => {
  ...
  if (origin) {
    ...
    if (!isBaseDomain) {
      ...
      const status = c.env.SECRETS_KV
        ? await c.env.SECRETS_KV.get(`DOMAIN:${originHost}`)
        : null;
      if (status !== "verified") {
      ...
      if (c.env.SECRETS_KV) {
        ...
        const currentReqs = parseInt(
          (await c.env.SECRETS_KV.get(rateKey)) || "0",
          10,
        );
        ...
        if (currentReqs >= 100) {
          ...
          if (c.env.DB) {
            await c.env.DB.prepare(
              "UPDATE api_domains SET status = 'blocked', blocked_reason = 'rate_limit_exceeded', updated_at = CURRENT_TIMESTAMP WHERE domain = ?",
            )
              .bind(originHost)
              .run();
          }
          console.warn(
            `[Security] Auto-blocked domain ${originHost} due to rate limiting.`,
          );
          c.res &&
            c.res.headers &&
            c.res.headers.set(
              "X-RateLimit-Warning",
              `Your domain ${originHost} has been auto-blocked due to exceeding rate limits`,
            );
          return c.text(
            "Too Many Requests. Domain automatically blocked due to spam activity.",
            429,
          );
        }

        await c.env.SECRETS_KV.put(rateKey, (currentReqs + 1).toString(), {
          expirationTtl: 60,
        });
      }
    }
...
app.get("/api/ai/gemini-stream/:workspaceId", async (c) => {
  ...
  try {
    ...
    if (geminiResponse.status !== 101) {
      console.error(
        `[Gemini Proxy] Failed to connect to Gemini: ${geminiResponse.status}`,
      );
...
app.get("/api/auth/me", async (c) => {
  ...
  if (c.env.SECRETS_KV) {
    ...
    return c.json({
      user: { email: "dev@dhitantra.local", id: "mock-user-123" },
    });
  }
...
app.post("/api/auth/send-otp", async (c) => {
  ...
  if (c.env.DB) {
    try {
      const existingUser: any = await c.env.DB.prepare(
        "SELECT id, is_registered, name FROM users WHERE email = ?",
      )
        .bind(email)
        .first();
      ...
      if (type === "register") {
        if (!existingUser) {
          ...
          await c.env.DB.prepare(
            "INSERT INTO users (id, email, name, is_registered) VALUES (?, ?, ?, 0)",
          )
            .bind(userId, email, name || "User")
            .run();
        } else {
          await c.env.DB.prepare("UPDATE users SET name = ? WHERE email = ?")
            .bind(name || existingUser.name || "User", email)
            .run();
        }
      }
    } catch (err) {
  ...
  if (c.env.SECRETS_KV) {
    ...
    if (inCooldown) {
      return c.json(
        {
          error:
            "कृपया एक और OTP का अनुरोध करने से पहले 60 सेकंड प्रतीक्षा करें।",
        },
        429,
      );
    }
  ...
  if (c.env.DB) {
    try {
      ...
      await c.env.DB.prepare("DELETE FROM otps WHERE email = ?")
        .bind(email)
        .run();
...
app.post("/api/user/settings", async (c) => {
  ...
  if (timezone) {
    ...
    if (c.env.SECRETS_KV) {
      await c.env.SECRETS_KV.put(`SESSION:${sessionId}`, JSON.stringify(user), {
...
app.post("/api/auth/verify-otp", async (c) => {
  ...
  if (c.env.DB) {
    try {
      ...
      const dbOtp: any = await c.env.DB.prepare(
        "SELECT * FROM otps WHERE email = ? AND otp_code = ? AND expires_at > ?",
      )
        .bind(email, otp, now)
        .first();
  ...
  if (c.env.DB) {
    try {
      ...
      if (existingUser) {
        ...
        const workspace: any = await c.env.DB.prepare(
        ...
        if (workspace) {
          ...
          await c.env.DB.prepare(
            ...
            .bind(defaultWorkspaceId, `${user.name || "My"} Workspace`)
            .run();
          await c.env.DB.prepare(
            "INSERT INTO workspace_members (workspace_id, user_id, role) VALUES (?, ?, ?)",
          )
            .bind(defaultWorkspaceId, user.id, "owner")
            .run();
        }
      } else {
        ...
        const userId = crypto.randomUUID();
        await c.env.DB.prepare(
          "INSERT INTO users (id, email, name, is_registered) VALUES (?, ?, ?, 1)",
        )
          .bind(userId, email, "User")
          .run();
        user.id = userId;
        user.name = "User";
        ...
        await c.env.DB.prepare(
          ...
          .bind(defaultWorkspaceId, `My Workspace`)
          .run();
        await c.env.DB.prepare(
          "INSERT INTO workspace_members (workspace_id, user_id, role) VALUES (?, ?, ?)",
        )
          .bind(defaultWorkspaceId, userId, "owner")
          .run();
      }
    } catch (err) {
...
app.get("/api/whatsapp/webhook", async (c) => {
  ...
  if (
    mode === "subscribe" &&
    token === (await c.env.SECRETS_KV.get("WHATSAPP_VERIFY_TOKEN"))
  ) {
        ...
        for (const change of entry.changes) {
          ...
          if (change.field === "calls") {
            ...
            if (!change.value || typeof change.value !== "object") {
              console.error(
                "[Calling] change.value is missing or not an object:",
                change,
              );
              continue;
            }
            ...
            console.log(
              `[Calling] ✅ calls field handler FIRED. phone_number_id from payload: ${change.value.metadata?.phone_number_id}`,
            );
            ...
            for (const callData of callsArray) {
              ...
              console.log(
                `[Calling] Event: ${event}, Call ID: ${callId}, From: ${callerNumber}, Direction: ${direction}, HasSDP: ${!!sdp}`,
              );

              const config = await c.env.DB.prepare(
                "SELECT workspace_id FROM whatsapp_configs WHERE phone_number_id = ?",
              )
                .bind(phoneNumberId)
              ...
              if (!config) {
                console.error(
                  `[Calling] ❌ No config found for phone_number_id: ${phoneNumberId}`,
                );
                continue;
              }

              console.log(
                `[Calling] Found workspace_id: ${config.workspace_id} for phone_number_id: ${phoneNumberId}`,
              );
              ...
              if (event === "connect" || event === "offer") {
                ...
                const existingContact = await c.env.DB.prepare(
                  ...
                  .bind(config.workspace_id, callerNumber)
                  .first<{ id: string }>();
              ...
              `,
                ...
                try {
                  const aiConfig = await c.env.DB.prepare(
                    "SELECT reply_mode, ai_voice_instructions FROM whatsapp_configs WHERE phone_number_id = ?",
                  )
                    .bind(phoneNumberId)
                    .first<{
                      reply_mode: string;
                  ...
                  if (
                    aiConfig &&
                    aiConfig.reply_mode === "ai" &&
                    aiConfig.ai_voice_instructions
                  ) {
                    routedToAI = true;
                    ...
                    c.executionCtx.waitUntil(
                      (async () => {
                        try {
                          const { initiateVoiceAgentBridge } =
                            await import("./services/voiceAgent");
                          await initiateVoiceAgentBridge(
                            c.env,
                            config.workspace_id,
                            callerNumber,
                            aiConfig.ai_voice_instructions,
                          ...
                          console.error(
                            "[Calling] Failed to trigger Voice Agent Bridge",
                            err,
                          );
                        }
                      })(),
                  ...
                  console.error(
                    "[Calling] Failed to check AI routing config",
                    e,
                  );
                }
                ...
                if (!routedToAI) {
                  try {
                    console.log(
                      `[Calling] Broadcasting to DO: global-${config.workspace_id} for Human Answering`,
                    );
                    const globalDoId = c.env.CHAT_DO.idFromName(
                      `global-${config.workspace_id}`,
                    );
                    const globalDo = c.env.CHAT_DO.get(globalDoId);
                    const broadcastResp = await globalDo.fetch(
                    ...
                    console.log(
                      `[Calling] ✅ Broadcast response from DO: ${broadcastBody}`,
                    );
                  } catch (e) {
                    console.error(
                      "[Calling] ❌ Failed to broadcast incoming call:",
                      e,
                    );
                  }
                }
                ...
                c.executionCtx.waitUntil(
                  (async () => {
                    try {
                      const aiConfig = await c.env.DB.prepare(
                        "SELECT ai_voice_instructions FROM whatsapp_configs WHERE phone_number_id = ?",
                      )
                        .bind(phoneNumberId)
                        .first<{ ai_voice_instructions: string }>();
                ...
                const existingCall = await c.env.DB.prepare(
                  "SELECT direction, status FROM calls WHERE id = ?",
                )
                  .bind(callId)
                  .first<{ direction: string; status: string }>();

                const wasMissed =
                  existingCall &&
                  existingCall.direction === "incoming" &&
                  existingCall.status === "ringing" &&
                  duration === 0;

                await c.env.DB.prepare(
                  "UPDATE calls SET status = ?, duration = ?, hangup_cause = ? WHERE id = ?",
                )
                  .bind(
                  ...
                  c.executionCtx.waitUntil(
                    (async () => {
                      try {
                        ...
                        console.error(
                          "[Calling] Failed to send missed call notification:",
                          e,
                        );
                      }
                    })(),
          ...
          if (change.value && change.value.messages) {
            ...
            if (message.text) {
              ...
              if (message.interactive.type === "button_reply") {
                ...
                messageText = message.interactive.nfm_reply?.response_json
              ...
              const contactName =
                message.contacts[0]?.name?.formatted_name || "Contact";
              const contactPhone =
              ...
              if (
                message.system &&
                message.system.type === "user_initiated_call"
              ) {
            ...
            console.log(
              `New ${messageType} message from ${contact?.profile?.name ?? "Unknown"} (${message.from}):`,
              messageText,
            );
              ...
              try {
                ...
                if (config && config.access_token) {
                  const res = await fetch(
                    `https://graph.facebook.com/v19.0/${mediaUrl}`,
                    {
                      headers: {
                    ...
                    if (config && config.workspace_id) {
                      ...
                      if (members.results && members.results.length > 0) {
                        ...
                        if (
                          emails.results &&
                          c.env.EMAIL_SENDER &&
                          typeof c.env.EMAIL_SENDER.send === "function"
                        ) {
                          const { EmailMessage } =
                            await import("cloudflare:email");
                          for (const row of emails.results) {
                            const senderEmail =
                              c.env.EMAIL_SENDER_ADDRESS ||
                              "dheetantra@navasanganakah.com";
            ...
            try {
              const msg = await c.env.DB.prepare(
                "SELECT id, conversation_id FROM messages WHERE platform_message_id = ?",
              )
                .bind(platformMsgId)
  ...
  if (c.env.DB) {
    try {
      const { results } = await c.env.DB.prepare(
        "SELECT * FROM api_domains WHERE workspace_id = ? ORDER BY created_at DESC",
      )
        .bind(workspaceId)
        .all();
...
app.post("/api/crm/api-domains", async (c) => {
  ...
  if (c.env.DB) {
    try {
      const id = crypto.randomUUID();
      await c.env.DB.prepare(
        "INSERT INTO api_domains (id, workspace_id, domain, status) VALUES (?, ?, ?, ?)",
      )
        .bind(id, workspaceId, cleanDomain, "pending")
        .run();
      return c.json({
        success: true,
        message: "Domain submitted for verification",
      });
    } catch (e: any) {
  ...
  if (c.env.DB) {
    try {
      await c.env.DB.prepare(
        ...
        .bind(crypto.randomUUID(), userId, token, device_type || "web")
        .run();
...
app.post("/api/crm/contacts", async (c) => {
  ...
  if (!phone)
    return c.json({ error: "फ़ोन नंबर आवश्यक है (Phone is required)" }, 400);
  ...
  const existing = await c.env.DB.prepare(
    ...
    .bind(workspaceId, platformContactId)
    .first();
...
app.post("/api/inbox/conversations/initiate", async (c) => {
  ...
  const contact = await c.env.DB.prepare(
    ...
    .bind(contactId, workspaceId)
    .first<{ platform_contact_id: string }>();
  if (!contact) return c.json({ error: "संपर्क नहीं मिला" }, 404);

  let finalPhoneNumberId = phone_number_id;
  if (!finalPhoneNumberId) {
    const config = await c.env.DB.prepare(
      "SELECT phone_number_id FROM whatsapp_configs WHERE workspace_id = ? ORDER BY created_at DESC LIMIT 1",
    )
      .bind(workspaceId)
      .first<{ phone_number_id: string }>();
    if (config) {
  ...
  let conv = await c.env.DB.prepare(
    "SELECT id FROM conversations WHERE workspace_id = ? AND contact_id = ? AND (phone_number_id = ? OR phone_number_id IS NULL)",
  )
    .bind(workspaceId, contactId, finalPhoneNumberId || "")
    .first<{ id: string }>();
...
app.post("/api/email-templates", async (c) => {
  ...
  await c.env.DB.prepare(
    ...
    .bind(id, workspaceId, templateType, subject, bodyHtml)
    .run();
  ...
  try {
    ...
    if (id) {
      existing = await c.env.DB.prepare(
        "SELECT id, waba_id, access_token, verify_token, reply_mode, ai_provider, ai_voice_instructions FROM whatsapp_configs WHERE id = ?",
      )
        .bind(id)
        .first();
    } else {
      existing = await c.env.DB.prepare(
        "SELECT id, waba_id, access_token, verify_token, reply_mode, ai_provider, ai_voice_instructions FROM whatsapp_configs WHERE workspace_id = ? AND phone_number_id = ?",
      )
        .bind(workspaceId, phone_number_id)
        .first();
    }

    const finalId = id || existing?.id || newId;
    const finalToken =
      access_token !== undefined ? access_token : existing?.access_token || "";
    const finalReplyMode =
      reply_mode !== undefined ? reply_mode : existing?.reply_mode || "manual";
    const finalAiProvider =
      ai_provider !== undefined
        ? ai_provider
        : existing?.ai_provider || "gemini";
    const finalAiVoiceInstructions =
      ai_voice_instructions !== undefined
        ? ai_voice_instructions
        : existing?.ai_voice_instructions || null;
    const finalWabaId =
      waba_id !== undefined ? waba_id : existing?.waba_id || null;
        ...
        (async () => {
          try {
            const enableRes = await fetch(
              `https://graph.facebook.com/v20.0/${phone_number_id}/settings`,
              {
                method: "POST",
                headers: {
            ...
            console.log(
              `[Calling] Auto-enabled calling for ${phone_number_id}:`,
              enableData,
            );
          } catch (e) {
            console.error(
              `[Calling] Failed to auto-enable calling for ${phone_number_id}:`,
              e,
            );
          }
          ...
          if (finalWabaId) {
            try {
              const subsRes = await fetch(
                `https://graph.facebook.com/v20.0/${finalWabaId}/subscribed_apps`,
                {
                  method: "POST",
                  headers: {
                    Authorization: `Bearer ${finalToken}`,
              ...
              console.log(
                `[Webhook] Subscribed messages+calls for WABA ${finalWabaId}:`,
                subsData,
              );
            } catch (e) {
              console.error(
                `[Webhook] Failed to subscribe for WABA ${finalWabaId}:`,
                e,
              );
            }
          }
...
app.get("/api/whatsapp/config", async (c) => {
  ...
  try {
    const { results } = await c.env.DB.prepare(
      "SELECT id, phone_number_id, waba_id, verify_token, reply_mode, calling_enabled, ai_provider, ai_voice_instructions, created_at FROM whatsapp_configs WHERE workspace_id = ?",
    )
      .bind(workspaceId)
      .all();
...
app.delete("/api/whatsapp/config/:id", async (c) => {
  ...
  try {
    await c.env.DB.prepare(
      "DELETE FROM whatsapp_configs WHERE id = ? AND workspace_id = ?",
    )
      .bind(id, workspaceId)
      .run();
    return c.json({
...
app.post("/api/whatsapp/templates", async (c) => {
  ...
  if (!name || !body_text)
    return c.json({ error: "Name and body text are required" }, 400);
  ...
  try {
    ...
    return c.json({
      success: true,
      message: metaSuccess
        ? "Template submitted to Meta and saved locally!"
...
app.delete("/api/whatsapp/templates/:id", async (c) => {
  ...
  try {
    await c.env.DB.prepare(
      "DELETE FROM whatsapp_templates WHERE id = ? AND workspace_id = ?",
    )
      .bind(id, workspaceId)
      .run();
...
app.post("/api/whatsapp/templates/send", async (c) => {
  ...
  const { to, templateName, languageCode, parameters, phoneNumberId } =
    await c.req.json();
  if (!to || !templateName)
  ...
  try {
    ...
    if (phoneNumberId) {
      config = await c.env.DB.prepare(
        "SELECT phone_number_id, access_token FROM whatsapp_configs WHERE workspace_id = ? AND phone_number_id = ?",
      )
        .bind(workspaceId, phoneNumberId)
        .first();
    }
    if (!config) {
      config = await c.env.DB.prepare(
        "SELECT phone_number_id, access_token FROM whatsapp_configs WHERE workspace_id = ?",
      )
        .bind(workspaceId)
        .first();
    }
    ...
    let contact = await c.env.DB.prepare(
      "SELECT id FROM contacts WHERE workspace_id = ? AND platform_contact_id = ?",
    )
      .bind(workspaceId, to)
      .first();
    ...
    if (!contactId) {
      ...
      await c.env.DB.prepare(
        ...
        .bind(contactId, workspaceId, "whatsapp", to, to)
        .run();
    }
    ...
    let conv = await c.env.DB.prepare(
      ...
      .bind(workspaceId, contactId, config.phone_number_id)
      .first<{ id: string }>();
    if (!conv) {
      ...
      if (conv) {
        await c.env.DB.prepare(
          "UPDATE conversations SET phone_number_id = ? WHERE id = ?",
        )
          .bind(config.phone_number_id, conv.id)
          .run();
      }
    }
    let convId = conv?.id;
    ...
    await c.env.DB.prepare(
      "INSERT INTO messages (id, conversation_id, sender_type, message_type, content, platform_message_id, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
    )
      .bind(
        msgId,
        convId,
...
app.post("/api/whatsapp/flows", async (c) => {
  ...
  try {
    ...
    if (id) {
      existing = await c.env.DB.prepare(
        "SELECT id FROM whatsapp_flows WHERE id = ? AND workspace_id = ?",
      )
        .bind(id, workspaceId)
        .first();
    }
...
app.delete("/api/whatsapp/flows/:id", async (c) => {
  ...
  try {
    await c.env.DB.prepare(
      "DELETE FROM whatsapp_flows WHERE id = ? AND workspace_id = ?",
    )
      .bind(id, workspaceId)
      .run();
...
app.post("/api/whatsapp/flows/:id/publish", async (c) => {
  ...
  try {
    await c.env.DB.prepare(
      "UPDATE whatsapp_flows SET status = 'PUBLISHED' WHERE id = ? AND workspace_id = ?",
    )
      .bind(id, workspaceId)
      .run();
...
app.post("/api/whatsapp/send", async (c) => {
  ...
  try {
    ...
    const isMediaId =
      mediaUrl && !mediaUrl.startsWith("http") && !mediaUrl.startsWith("/");
    ...
    await c.env.DB.prepare(
      "INSERT INTO messages (id, conversation_id, sender_type, message_type, content, media_url, platform_message_id, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
    )
      .bind(
        savedMessageId,
        conversationId,
...
app.post("/api/inbox/conversations/:conversationId/status", async (c) => {
  ...
  const conv = await c.env.DB.prepare(
    ...
    .first();
  if (!conv)
    return c.json({ error: "Conversation not found or forbidden" }, 404);
...
app.post("/api/whatsapp/calls", async (c) => {
  ...
  const contact = await c.env.DB.prepare(
    "SELECT name, platform_contact_id FROM contacts WHERE id = ?",
  )
    .bind(contactId)
    .first<{ name: string; platform_contact_id: string }>();
...
app.post("/api/whatsapp/calls/:id/status", async (c) => {
  ...
  await c.env.DB.prepare(
    "UPDATE calls SET status = ?, duration = COALESCE(?, duration) WHERE id = ? AND workspace_id = ?",
  )
    .bind(status, duration, callId, workspaceId)
    .run();
...
app.post("/api/whatsapp/calls/:id/answer", async (c) => {
  ...
  if (!config) {
    config = await c.env.DB.prepare(
      "SELECT access_token FROM whatsapp_configs WHERE workspace_id = ?",
    )
      .bind(workspaceId)
      .first<{ access_token: string }>();
  }
  ...
  await c.env.DB.prepare(
    "UPDATE calls SET status = ? WHERE id = ? AND workspace_id = ?",
  )
    .bind("in_progress", callId, workspaceId)
    .run();

  return c.json({
    success: true,
    preAccept: preAcceptData,
    accept: acceptData,
  });
});
...
app.post("/api/whatsapp/calls/:id/reject", async (c) => {
  ...
  await c.env.DB.prepare(
    ...
    .bind("declined", callId, workspaceId)
    .run();
...
app.post("/api/whatsapp/calls/toggle", async (c) => {
  ...
  await c.env.DB.prepare(
    "UPDATE whatsapp_configs SET calling_enabled = ? WHERE workspace_id = ?",
  )
    .bind(enabledValue, workspaceId)
    .run();
    ...
    (async () => {
      try {
        const configs = await c.env.DB.prepare(
          ...
          .bind(workspaceId)
          .all<{ phone_number_id: string; access_token: string }>();
        if (configs.results) {
          for (const cfg of configs.results) {
            await fetch(
              `https://graph.facebook.com/v20.0/${cfg.phone_number_id}/settings`,
              {
                method: "POST",
                headers: {
...
app.get("/api/webrtc/ice-servers", async (c) => {
  try {
    ...
    const turnKeyId =
      (await c.env.SECRETS_KV.get("CLOUDFLARE_CALLS_APP_ID")) ||
      (await c.env.SECRETS_KV.get("TURN_KEY_ID"));
    const turnToken =
      (await c.env.SECRETS_KV.get("CLOUDFLARE_API_TOKEN")) ||
      (await c.env.SECRETS_KV.get("TURN_KEY_API_TOKEN"));
...
app.post("/api/whatsapp/calls/re-enable", async (c) => {
  ...
  const config = await c.env.DB.prepare(
    ...
    .bind(phoneNumberId, workspaceId)
    .first<{ access_token: string }>();
...
app.get("/api/whatsapp/calls/config", async (c) => {
  ...
  const config = await c.env.DB.prepare(
    "SELECT calling_enabled FROM whatsapp_configs WHERE workspace_id = ?",
  )
    .bind(workspaceId)
    .first<{ calling_enabled: number }>();

  return c.json({
    calling_enabled: config ? config.calling_enabled === 1 : true,
  });
});
...
app.get("/api/whatsapp/calls/status", async (c) => {
  ...
  if (firstConfig) {
    try {
      ...
      if (wabaRow && wabaRow.waba_id) {
        const subsRes = await fetch(
          `https://graph.facebook.com/v20.0/${wabaRow.waba_id}/subscribed_apps`,
          {
        ...
        if (subsData.data && subsData.data.length > 0) {
          const fields =
            subsData.data[0].subscribed_fields ||
            subsData.data[0].whatsapp_business_api_data?.subscribed_fields ||
            [];
          webhookCallsFieldHint =
            Array.isArray(fields) && fields.includes("calls");
        }
        ...
        if (!webhookCallsFieldHint) {
          try {
            ...
            console.log(
              `[Calling Status] Auto-fix webhook subscription for WABA ${wabaRow.waba_id}:`,
              fixData,
            );
            ...
            console.error(
              "[Calling Status] Auto-fix webhook subscription failed:",
              fixErr,
            );
          }
        }
      ...
      console.error(
        "[Calling Status] Failed to check webhook subscription:",
        e,
      );
    }
  }
  ...
  return c.json({
    ...
    all_ready:
      phoneResults.every((p) => p.db_calling_enabled) && webhookCallsFieldHint,
  });
});
...
app.post("/api/whatsapp/webhook/subscribe", async (c) => {
  ...
  try {
    const config = await c.env.DB.prepare(
      ...
      .bind(workspaceId)
      .first<{ waba_id: string; access_token: string }>();

    if (!config || !config.waba_id) {
      return c.json(
        {
          error:
            "WABA ID नहीं मिला। कृपया पहले WhatsApp Config में WABA ID सेव करें।",
        },
        400,
      );
    }
    ...
    const subsRes = await fetch(
      `https://graph.facebook.com/v20.0/${config.waba_id}/subscribed_apps`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${config.access_token}`,
    ...
    const subsData: any = await subsRes.json();
    console.log(
      `[Webhook Subscribe] Manual subscription for WABA ${config.waba_id}:`,
      subsData,
    );
    ...
    if (subsData.success === true) {
      return c.json({
        success: true,
        message:
          "Webhook fields (messages + calls) सफलतापूर्वक subscribe हो गए!",
      });
    } else {
...
app.post("/api/broadcast", async (c) => {
  ...
  if (
    ...
    return c.json(
      {
        error:
          "Missing required fields: campaignName, templateName, contactIds",
      },
      400,
    );
  }
  ...
  try {
    ...
    const { results: contacts } = await c.env.DB.prepare(
      ...
      .all<{ id: string; platform_contact_id: string; phone: string }>();
    ...
    for (const contact of contacts) {
      ...
      if (!queueAvailable) {
        console.error(
          "[broadcast] BROADCAST_QUEUE binding not configured — skipping queue",
        );
        break;
      }
      try {
        ...
        console.error(
          `Failed to queue broadcast for contact ${contact.id}:`,
          qErr,
        );
      }
    }
    ...
    if (!queueAvailable) {
      await c.env.DB.prepare(
        ...
        .bind("failed", campaignId)
        .run();
      return c.json(
        {
          success: false,
          error:
            "Broadcast queue not configured. Run: wrangler queues create broadcast-queue-prod",
          campaignId,
          total: 0,
        },
        500,
...
app.get("/api/plans", async (c) => {
  ...
  try {
    const { results } = await c.env.DB.prepare(
      "SELECT * FROM plans ORDER BY upfront_price ASC",
    ).all();
...
app.get("/api/whatsapp/media", async (c) => {
  ...
  try {
    ...
    const graphUrl = mediaUrl.startsWith("http")
      ? mediaUrl
      : `https://graph.facebook.com/v19.0/${mediaUrl}`;
    ...
    headers.set(
      "Content-Type",
      binaryRes.headers.get("Content-Type") || "application/octet-stream",
    );
  ...
  try {
    ...
    try {
      await c.env.DB.prepare(
        ...
        .bind(name.trim(), contactEmail.trim(), message.trim())
        .run();
    } catch (dbErr) {
...
app.notFound((c) => {
  return c.json(
    {
      error: "Not Found",
      message: "API route or static asset does not exist.",
    },
    404,
  );
});
