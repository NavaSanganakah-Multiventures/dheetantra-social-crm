...
router.post('/api/whatsapp/webhook/subscribe', async (c) => {
  ...
  try {
    ...
    if (!config || !config.waba_id) {
      return c.json({ error: 'WABA ID à¤¨à¤¹à¥à¤ à¤®à¤¿à¤²à¤¾à¥¤ à¤à¥à¤ªà¤¯à¤¾ à¤ªà¤¹à¤²à¥ WhatsApp Config à¤®à¥à¤ WABA ID à¤¸à¥à¤µ à¤à¤°à¥à¤à¥¤' }, 400);
    }
    ...
    if (subsData.success === true) {
      return c.json({ success: true, message: 'Webhook fields (messages + calls) à¤¸à¤«à¤²à¤¤à¤¾à¤ªà¥à¤°à¥à¤µà¤ subscribe à¤¹à¥ à¤à¤!' });
    } else {
...
// Broadcast Campaign â list campaigns for the workspace
...
// Broadcast Campaign â Create campaign and queue messages.
// Supports two modes:
...
router.post('/api/broadcast', requireRole('owner', 'admin'), async (c) => {
  ...
  if (resolvedContactIds.length === 0 && !templateName && message) {
    // Text-mode broadcasts go out over WhatsApp only â email contacts have an
  ...
  try {
    ...
    if (!queueAvailable) {
      console.error('[broadcast] BROADCAST_QUEUE binding not configured â skipping queue');
    }
