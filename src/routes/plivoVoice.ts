...
import { plivoStreamHandler } from '../services/plivoStream';

// ---------------------------------------------------------------------------
// Plivo voice provider (PSTN bridge). Mirrors twilioVoice.ts but talks to the
// Plivo Voice API with raw fetch (no Plivo SDK).
//
// Call modes (per Plivo account):
//   * Voice Bot ON  (voice_bot_enabled=1): the inbound webhook returns a
//     <Stream> element that bridges the caller to the Gemini Live AI assistant
//     ("Arya") via /plivo-stream/:callId. No agent PSTN leg is dialed, so only
//     the single inbound caller leg is billed -> lowest cost, lowest latency.
//   * Voice Bot OFF + auto_dial_agents ON : legacy fallback bridge. The caller
//     and the agent's PSTN phone are dialed into the same Plivo conference
//     (two PSTN legs -> higher cost/latency).
//   * Voice Bot OFF + auto_dial_agents OFF : in-app (softphone) answering. Only
//     the customer leg is fired; the agent joins the conference via the
//     browser/Flutter SIP softphone (free agent leg).
// ---------------------------------------------------------------------------
...
// Convert an https base URL to a wss URL for Plivo <Stream>.
function getStreamWsUrl(c: Context, callId: string): string {
  const base = getBaseUrl(c).replace(/^http:/, 'ws:').replace(/^https:/, 'wss:');
  return base + '/plivo-stream/' + callId;
}
...
// ---------------------------------------------------------------
// Voice Bot Stream WebSocket bridge (no auth; Plivo connects server-to-server)
// ---------------------------------------------------------------
router.get('/plivo-stream/:callId', plivoStreamHandler as any);
...
router.get('/api/plivo/configs', async (c) => {
  ...
  const { results } = await c.env.DB.prepare(
    'SELECT id, name, auth_id, auth_token, is_active, auto_dial_agents, endpoint_username, endpoint_password, voice_bot_enabled, voice_bot_instructions, voice_bot_greeting, office_hours_start, office_hours_end, office_hours_audio_url, busy_audio_url, created_at, updated_at FROM plivo_configs WHERE workspace_id = ? ORDER BY created_at ASC'
  ).bind(workspaceId).all<{ id: string; name: string; auth_id: string; auth_token: string; is_active: number; auto_dial_agents: number; endpoint_username: string | null; endpoint_password: string | null; voice_bot_enabled: number; voice_bot_instructions: string | null; voice_bot_greeting: string | null; office_hours_start: string; office_hours_end: string; office_hours_audio_url: string | null; busy_audio_url: string | null; created_at: string; updated_at: string }>();
  ...
  for (const cfg of results || []) {
    ...
    configs.push({
      ...
      voiceBotInstructions: cfg.voice_bot_instructions ?? '',
      voiceBotGreeting: cfg.voice_bot_greeting ?? '',
...
router.post('/api/plivo/configs', requireRole('owner', 'admin'), async (c) => {
  ...
  const { name, authId, authToken, fromNumbers, autoDialAgents, endpointUsername, endpointPassword, voiceBotEnabled, voiceBotInstructions, voiceBotGreeting, officeHoursStart, officeHoursEnd, officeHoursAudioUrl, busyAudioUrl } = await c.req.json() as any;
  if (!authId || !authToken) {
  ...
  await c.env.DB.prepare(
    'INSERT INTO plivo_configs (id, workspace_id, name, auth_id, auth_token, is_active, auto_dial_agents, endpoint_username, endpoint_password, voice_bot_enabled, voice_bot_instructions, voice_bot_greeting, office_hours_start, office_hours_end, office_hours_audio_url, busy_audio_url, created_at, updated_at) VALUES (?, ?, ?, ?, ?, 1, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
  ).bind(
    id, workspaceId, name || 'My Plivo Account', authId, authToken,
    autoDialAgents === true ? 1 : 0,
    endpointUsername || null, endpointPassword || null,
    voiceBotEnabled === false ? 0 : 1,
    voiceBotInstructions || null, voiceBotGreeting || null,
    officeHoursStart || '09:00', officeHoursEnd || '16:00',
...
router.put('/api/plivo/configs/:id', requireRole('owner', 'admin'), async (c) => {
  ...
  if (body.voiceBotInstructions !== undefined) { updates.push('voice_bot_instructions = ?'); params.push(body.voiceBotInstructions || null); }
  if (body.voiceBotGreeting !== undefined) { updates.push('voice_bot_greeting = ?'); params.push(body.voiceBotGreeting || null); }
...
// Incoming PSTN call -> either the AI Voice Bot (<Stream>) or a conference
// waiting room + dial an available agent.
router.post('/api/plivo/webhook/voice', async (c) => {
  try {
    ...
    const config = await c.env.DB.prepare(
      'SELECT tc.id AS plivo_config_id, tc.workspace_id, tc.auth_id, tc.auth_token, tc.auto_dial_agents, tc.voice_bot_enabled, tc.voice_bot_instructions, tc.voice_bot_greeting, tc.office_hours_start, tc.office_hours_end, tc.office_hours_audio_url, tc.busy_audio_url, tfn.id AS from_number_id, tfn.from_number FROM plivo_configs tc JOIN plivo_from_numbers tfn ON tc.id = tfn.plivo_config_id WHERE tfn.from_number IN (?, ?, ?) AND tfn.is_active = 1 AND tc.is_active = 1 LIMIT 1'
    ).bind(candidates[0], candidates[1], candidates[2]).first<{ plivo_config_id: string; workspace_id: string; auth_id: string; auth_token: string; auto_dial_agents: number; voice_bot_enabled: number; voice_bot_instructions: string | null; voice_bot_greeting: string | null; office_hours_start: string; office_hours_end: string; office_hours_audio_url: string | null; busy_audio_url: string | null; from_number_id: string; from_number: string }>();
    ...
    if (config.voice_bot_enabled === 1) {
      // ---- Voice Bot mode: the AI assistant ("Arya") answers the call ----
      // Office hours gate: outside business hours, play a message and hang up.
      const now = new Date();
      ...
      if (currentIstMinutes < startMinutes || currentIstMinutes >= endMinutes) {
        ...
        await c.env.DB.prepare("UPDATE calls SET status = 'missed' WHERE id = ?").bind(callId).run();
        return plivoXmlResponse(xml, 200);
      }

      // Inside office hours: bridge the caller to the Gemini Live AI assistant
      // via a bidirectional <Stream>. No agent PSTN leg is dialed, so only the
      // single inbound caller leg is billed (lowest cost) and the AI handles
      // the whole conversation with low latency.
      const streamUrl = getStreamWsUrl(c as Context, callId);
      const greeting = (config.voice_bot_greeting && config.voice_bot_greeting.trim())
        || 'Namaste, main Arya hoon. Aapse baat karke khushi hui. Bataiye, main aapki kaise madad kar sakta hoon?';

      const xml = XML_DECL +
        '<Response>' +
        '<Speak language="hi-IN" voice="Polly.Aditi">' + escXml(greeting) + '</Speak>' +
        '<Stream bidirectional="true" keepCallAlive="true" contentType="audio/x-mulaw;rate=8000">' + escXml(streamUrl) + '</Stream>' +
        '</Response>';

      await c.env.DB.prepare("UPDATE calls SET status = 'in_progress' WHERE id = ?").bind(callId).run();
      await broadcastToWorkspace(c.env, config.workspace_id, { type: 'call_status_updated', call_id: callId, status: 'in_progress', source: 'plivo', voiceBot: true });
      return plivoXmlResponse(xml, 200);
    }

    // ---- Non-bot mode: assign an available (live) agent, if any. Fire the
    // agent leg first and only mark them busy once the leg was accepted. ----
    ...
    // in the conference and agents can answer in the app (softphone).
